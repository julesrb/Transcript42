...
\newcommand\BackgroundPic{
  \begin{tikzpicture}[remember picture, overlay]
    \node at (current page.center) [
      ...
      {%- if campus_id == 51 %}
      \includegraphics[width=\paperwidth, height=\paperheight]{/app/graphics/42_BERLIN_back.pdf}
      {%- else %}
      \includegraphics[width=\paperwidth, height=\paperheight]{/app/graphics/42_back.pdf}
      {%- endif %}
	  };
  \end{tikzpicture}
}
...
\fancyhead[L]{
    ...
    \parbox[t]{\hsize}{
		{%- if campus_id == 51 %}
		\raisebox{-37pt}{\includegraphics[width=4.1cm]{/app/graphics/42_BERLIN_logo.png}}
		{%- else %}
		\raisebox{-47pt}{\includegraphics[width=2.1cm]{/app/graphics/42_logo.png}}
		{%- endif %}
		\hspace*{-1.2cm}
		\begin{minipage}[t]{\dimexpr\hsize-3cm\relax}
			\vspace{1.03cm}
	...
	\parbox[t]{\hsize}{
		...
		{%- else %}
			\textbf{\large TRANSCRIPT OF ACADEMIC RECORDS} \\[0.5em]
		{%- endif %}
		...
		\begin{tabular}{@{}l l@{}}
			{%- if language == 'de' %}
				...
				Auswahl bestanden in: & {{ passed_selection }} \\
			...
			{%- else %}
				...
				Passed Selection in: & {{ passed_selection }} \\
				Core Curriculum started on: & {{ core_started }} \\
				Core Curriculum completed on: & {{ core_completed }} \\
				{% if transcript_type == 'core_advanced' and core_completed != "in progress" -%}
